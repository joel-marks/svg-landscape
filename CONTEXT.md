...
5.13. Push to `origin` restored. Ten local commits (everything through 5.12) had never reached GitHub — the remote repo was empty, so the Pages deploy workflow had never run once despite being correct since the scaffold phase. Diagnosis: `origin` was configured correctly and `branch.main` had its upstream recorded in `.git/config` all along; the missing piece was the remote branch itself, so `origin/main` read as `gone`. Fixed with `git push -u origin main`. `CLAUDE.md` gained a "Version control discipline" section making commit → report hash → push → confirm push succeeded explicit per-phase requirements, since this went unnoticed across nine phases.
...
- Nothing ever pushed to `origin` (found Phase 5.13). The remote was configured correctly the whole time and the upstream was set in `.git/config`; the remote repository itself was simply empty, which is why `git status` reported `origin/main: gone` rather than "no upstream." Consequence was that the section 12 deploy pipeline had never executed — local commits looked like done work but nothing was live. Fixed by `git push -u origin main`. Guard against recurrence is procedural, not technical: the `CLAUDE.md` version-control section now requires confirming the push, not just the commit.