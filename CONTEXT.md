...
- Tips: on/off. Scope: a short one-line caption under each folder heading (Scene, Canvas, Lighting, Color, Actions) explaining what that group does — not per-control tooltips, which is a larger build for marginal benefit over folder-level captions. Hidden entirely when off, not just greyed.
...
**Open**
- Slider values can land an ulp off the value that was written to them (0.5 stored as 0.49999999999999994, 0 as 1.7e-18). Tweakpane's step constraint is `origin % step + round((value - origin) / step) * step` with **origin fixed at the value the pane was constructed with** — so writing a value back through a slider that has since moved can miss by a float ulp. Pre-existing (it applied to preset loading before Phase 6) but newly visible in two places: the persisted blob now carries such values, and the pane's construction origin is now a restored value rather than always the factory default. Invisible in the panel (displayed to 2dp) and in the render (~1e-17), and stable rather than drifting under repeated resets. The real risk is `currentPresetId()`, which compares with `===`: a preset whose values differ from the live scene only by an ulp would show as "Custom". Not reproduced in testing — the epsilon didn't appear in the direction that would trigger it — so it is recorded here rather than fixed, since the fix (a tolerance in `sameValue`, or rounding in `exportSettings`) changes the Phase 5.12-finalized export/match mechanism and should be a deliberate call.
