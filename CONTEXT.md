...
- Random scene (Phase 6.6) — beside New View, and its mirror image: it redraws Complexity, Peak count, Peak sharpness and Point of view height to fresh values in their existing ranges while keeping the seed, where New View keeps every parameter and changes only the seed. Landscape type is deliberately not included — rerolling that too would make it a "surprise me" for the whole panel, and the archetype is the one Scene choice a user has usually already made on purpose. The seed lock has no bearing on it: the lock guards against *incidental* reseeding and this never reseeds at all, so there is nothing for it to suppress.
...
**Preferences** — Right column, in its own folder alongside Actions (never assigned a column before Phase 6 — closed here, same pattern as the earlier Lighting/Canvas gaps).
...
**Open**
- The Presets panel is specified in section 5 as sitting "between the header and the canvas frame" and has never actually been built there — it has been a pane stacked at the top of the left column since Phase 5.7. Noticed in Phase 6.6 while adding the panel's heading; not resolved there, because which of the two is wrong is a layout decision rather than a bug fix. Either move the panel or amend the sentence.
